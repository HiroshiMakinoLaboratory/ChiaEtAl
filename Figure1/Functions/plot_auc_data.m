%% Plot AUC data for single cells
function plot_auc_data(parameters,roc_data,test_epoch)
concat_ROC_data = roc_data.concat_auc;
concat_shuffled_auc_data = roc_data.concat_shuffled_auc_data;
...
for session_num = 1:3
    for region_num = 1:8
        concat_ROC_data{session_num}{region_num} = concat_ROC_data{session_num}{region_num} - nanmean(nanmean(concat_shuffled_auc_data{session_num}{region_num}(:,baseline_epoch,:),2),3);
        concat_shuffled_auc_data{session_num}{region_num} = concat_shuffled_auc_data{session_num}{region_num} - nanmean(nanmean(concat_shuffled_auc_data{session_num}{region_num}(:,baseline_epoch,:),2),3);
    end
end
